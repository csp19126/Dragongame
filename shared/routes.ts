  ...
  app.post(api.game.spin.path, async (req: any, res) => {
    try {
      ...
      PAYLINES.forEach((line, idx) => {
        ...
        // Check if line is an array of coordinates [[c,r], [c,r], [c,r]]
        if (Array.isArray(line[0])) {
          vals = (line as [number, number][]).map(([col, row]) => {
            return (grid[col] && grid[col][row]) ? grid[col][row] : null;
          });
        } else {
          // If line is just [0,0,0] or [1,1,1] (horizontal rows)
          vals = [grid[0][line[0] as any], grid[1][line[1] as any], grid[2][line[2] as any]];
        }
      ...
      await storage.updateGameState(user.id, input.slotId, { 
        ...
        consecutiveWins: winAmount > 0 ? (gameState?.consecutiveWins ?? 0) + 1 : 0
      });
      ...
      const updatedUser = await storage.updateStreak(
        user.id, 
        winAmount > 0 ? (gameState?.consecutiveWins ?? 0) + 1 : 0, 
        user.maxStreak ?? 0, 
      ...
      res.json({
        ...
        streak: winAmount > 0 ? (gameState?.consecutiveWins ?? 0) + 1 : 0,
